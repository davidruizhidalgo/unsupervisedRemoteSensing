#Nuevo script para entrenamiento de una red inception utilizando clasificadores LR, SVM y RIEMANNIAN
...
for i in range(0, numTest):
    ...
    #logger_LRC.savedataTrain(history)
    #GUARDAR MODELO DE RED CONVOLUCIONAL
    cnn_net = Model(inputs = lrc_net.input, outputs = lrc_net.layers[-4].output)
    cnn_net.save(os.path.join(logger_LRC.path,test+str(i)+'.h5'))
    #GUARDAR MODELO DE RED CONVOLUCIONAL + LRC
    lrc_net.save(os.path.join(logger_LRC.path,test+str(i)+'_LRC.h5'))
    #EVALUAR MODELO
    ...
    logger_LRC.savedataPerformance(OA, AA, kappa)
    #############################################################################################################################################################
    ...
    joblib.dump(rie_net, os.path.join(logger_RIE.path,test+'_RIE'+str(i)+'.pkl')) 
    #EVALUAR MODELO
    #GENERACION OA - Overall Accuracy 
    ...
    kappa = cohen_kappa_score(etiquetasPrueba, datosSalida)
    #LOGGER DATOS DE VALIDACIÓN
    logger_RIE.savedataPerformance(OA, AA, kappa)
    #############################################################################################################################################################
    ################################## CLASIFICADOR SVM #########################################################################################################
    datosPrueba, etiquetasPrueba = preparar.extraerDatosPrueba2D(ventana)
    #Crear y entrenar clasificador 
    ...
    #Guarda el clasificador entrenado
    joblib.dump(svm_net, os.path.join(logger_SVM.path,test+'_SVM'+str(i)+'.pkl')) 
    #EVALUAR MODELO
    #GENERACION OA - Overall Accuracy 
    ...
    kappa = cohen_kappa_score(etiquetasPrueba, datosSalida)
    #LOGGER DATOS DE VALIDACIÓN
    logger_SVM.savedataPerformance(OA, AA, kappa)
    #############################################################################################################################################################
    #######################RETORNAR ETIQUETAS DE PRUEBA Y PREDICCIÓN#############################################################################################
    class_names = []
    for j in range(1,groundTruth.max()+1):
        class_names.append('Class '+str(j))
    ...
    data.saveHsi_VS(groundTruth, imagenSalida_LRC, os.path.join(logger_LRC.path, 'cmap'+str(i)+'.png'))
    data.saveHsi_VS(groundTruth, imgCompare_LRC, os.path.join(logger_LRC.path, 'compare'+str(i)+'.png'), cmap ='bw')
    data.saveHsi_VS(groundTruth, imagenSalida_RIE, os.path.join(logger_RIE.path, 'cmap'+str(i)+'.png'))
    data.saveHsi_VS(groundTruth, imgCompare_RIE, os.path.join(logger_RIE.path, 'compare'+str(i)+'.png'), cmap ='bw')
    data.saveHsi_VS(groundTruth, imagenSalida_SVM, os.path.join(logger_SVM.path, 'cmap'+str(i)+'.png'))
    data.saveHsi_VS(groundTruth, imgCompare_SVM, os.path.join(logger_SVM.path, 'compare'+str(i)+'.png'), cmap ='bw')
