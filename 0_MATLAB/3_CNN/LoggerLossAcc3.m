...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
data = load('Logger/6_KPCA_CNN/logger_IndianPines.txt');
x=1:size(data,2);
loss=zeros(10,size(data,2)); j=1;
for i=1:4:40
   loss(j,:) = data(i,:);
...
legend('PCA+CNN','KPCA+CNN','KPCA+Inceprion')
title('Función de Costo'); ylabel('loss')
subplot(2,1,2)
plot(x,acc,'-p'); grid on
legend('PCA+CNN','KPCA+CNN','KPCA+Inceprion','Location','SouthEast')
title('Precisión Entrenamiento'); ylabel('loss'); xlabel('epoch')
